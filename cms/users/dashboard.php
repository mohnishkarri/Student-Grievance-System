...
<?php
$query=mysqli_query($con,"select id from users where userEmail='".$_SESSION['login']."'");
while($row=mysqli_fetch_array($query))
{
$userid = $row[0];
}
$sql1=mysqli_query($con,"SELECT c.complaintNumber,c.status,c.remark from tblcomplaints t, complaintremark c WHERE t.userId='$userid' and c.complaintNumber=t.complaintNumber;");
while($rows=mysqli_fetch_array($sql1))
{
?>

<p class="h3 text-center mb-3 text-success">Complaint details</p>
<table class="table" >
<tr>
...
?>
         <!-- $sql = mysqli_query("SELECT * FROM tblcomplaints t  where userId='".$_SESSION['id']."' and  status='$status'" );?>
              </section> -->
