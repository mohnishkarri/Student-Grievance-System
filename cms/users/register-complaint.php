...
include('C:\xampp\htdocs\sgs2\cms\sm.js');
...
<html lang="en">
  <head>
    ...
    <title>SGS | User Register Complaint</title>
...
<?php $sql=mysqli_query($con,"select id,category from college ");

while ($rw=mysqli_fetch_array($sql)) {
  ?>
 
  <option value="<?php echo htmlentities($rw['id']);?>"><?php echo htmlentities($rw['category']);?></option>
<?php
}
?>
</select>
...
<div class="form-group">
<label class="col-sm-2 col-sm-2 control-label">Complaint Level</label>
<div class="col-sm-4">
<select name="complaintype" class="form-control" required="">
                <option value=" Complaint"> Complaint</option>
                <?php $sql=mysqli_query($con,"select lname from level");
while ($rw=mysqli_fetch_array($sql)) {
  ?>
  <option value="<?php echo htmlentities($rw['lname']);?>"><?php echo htmlentities($rw['lname']);?></option>
<?php
}
?>
...
<option id="dept" value="">Select Department</option>
<?php $sql=mysqli_query($con,"select subcategory from subcategory,category where subcategory.categoryid=category.id");
while ($rw=mysqli_fetch_array($sql)) {
 ?>
  <option value="<?php echo htmlentities($rw['id']);?>"><?php echo htmlentities($rw['category']);?></option>
<?php
...
<!-- 
<script>
document.getElementbyId('dept').value = clg;
</script> -->
